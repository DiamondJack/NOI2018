...
int dfn[N<<1],slen;
int ls[N*80],rs[N*80],sum[N*80],rt[N<<1],segcnt,len[N<<1],Fa[20][N<<1];
int *fa=Fa[0];
...
void askquery(){
	...
	REP(i,1,Alen){
		...
		if(!cur){
			cur=1; curlen=0; continue; //cout<<0<<endl; continue;
		}
	...
	//REP(i,1,tot) printf("%d ",seqA[i]); puts("");
	REP(i,1,tot-1){
		int k3=0;
	...
	long long ans3=sam2::count(); //cout<<ans3<<endl;
	ans=ans3-ans;
	printf("%lld\n",ans);
}
...
int main(int argc,char** argv){
	...
	scanf("%s",s+1);
	slen=strlen(s+1);
	REP(i,1,slen) assert('a'<=s[i]&&s[i]<='z');
	assert(SLEN*0.8 <= slen && slen<=SLEN);
	//printf("%d\n",slen);
	...
	int q; scanf("%d",&q);
	//assert(QRANGE * 0.8 <= q && q <= QRANGE);
//	cerr<<q<<endl;
	int lensum=0;
	while(q--){
		scanf("%s%d%d",A+1,&ql,&qr);
		assert(1<=ql&&ql<=qr&&qr<=slen);
		if(All){
			assert(ql==1);
			assert(qr==slen);
		}
		Alen=strlen(A+1);
		assert(Alen <= MAXTLEN);
		REP(i,1,Alen) assert('a'<=A[i]&&A[i]<='z');
		lensum+=Alen;
		askquery();
	}
	
	cerr<<lensum<<" "<<TLEN<<" "<<endl;
	assert(TLEN * 0.8 <= lensum && lensum<=TLEN);
